...
export const API_KEY =
  'AAPK1241ad7e316e4489976f61c0af02491bYPPiBRS49FnRYvOBlFotefdVTVHZr2CddbDhLyqPrADa-lrrFLP0-e96rtW1vpSP'
...
export const genMarkerIcon = (symbol: string, size: string, color: string) => {
  ...
  const colorCode =
    color && color.startsWith('#') ? color.replace('#', '') : color
  const iconUrl = `${ASSET_URL}/${
    symbolSize?.value ? symbolSize?.value : 'pin-m'
  }${symbol ? '-' + symbol : ''}+${
    colorCode ? colorCode : '7e7e7e'
